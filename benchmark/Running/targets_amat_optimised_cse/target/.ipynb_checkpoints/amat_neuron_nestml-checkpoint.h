...
static std::vector< std::tuple< int, int > > rport_to_nestml_buffer_idx;
  ...
  **/
  struct Variables_
  {
    ...
    // coefficient multiplying (I_e + I_stim) external/input in the analytical V_m update. 
    double __P__V_m__I_input;

    // constant offset term in the closed-form V_m propagator. 
    double __P__V_m__E_L_offset; // contribution of the leak/reversal potential E_L to the analytical V_m update
