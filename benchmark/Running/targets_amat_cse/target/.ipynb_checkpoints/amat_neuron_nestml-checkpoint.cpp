 ...
 *
 *  Generated from NESTML 8.3.0-rc3-post-dev at time: 2026-08-19 09:48:42.352566
**/
...
void amat_neuron_nestml::recompute_internal_variables(bool exclude_timestep)
{
  ...
  if (exclude_timestep) // In NEST Simulator code generated by NESTML, the exclude_timestep parameter in recompute_internal_variables is used to prevent the duplicate  application of the simulation timestep when resetting variables immediately after an event (like a spike)
  {    
    V_.unit_psc = 1; // as pA
    ...
    // PRECOMPUTED COEFFICIENTS (branch 01 for refractory?) 
    // CSE - compute new coefficients once.  These only depend on parameters (tau_m, C_m, E_L) and the timestep-derived propagator __P__V_m__V_m
    V_.__P__V_m__I_input = (P_.tau_m / P_.C_m) * (1.0 - V_.__P__V_m__V_m);
    V_.__P__V_m__E_L_offset = P_.E_L * (1.0 - V_.__P__V_m__V_m);
    // This removes 4 divisions and 2 multiplies from the hot loop entirely — they now run only when the simulation resolution or tau_m/C_m/E_L change (i.e. essentially never, compared to once per timestep per neuron). can we maybe restructure the code so we dont need this? 


      // EXISTING COEFFICENTS 
    ...
    // PRECOMPUTED COEFFICIENTS (branch 01 for non-refractory?) 
    // CSE - compute new coefficients once.  These only depend on parameters (tau_m, C_m, E_L) and the timestep-derived propagator __P__V_m__V_m
    V_.__P__V_m__I_input = (P_.tau_m / P_.C_m) * (1.0 - V_.__P__V_m__V_m);
    V_.__P__V_m__E_L_offset = P_.E_L * (1.0 - V_.__P__V_m__V_m);
    // This removes 4 divisions and 2 multiplies from the hot loop entirely — they now run only when the simulation resolution or tau_m/C_m/E_L change (i.e. essentially never, compared to once per timestep per neuron).
...
void amat_neuron_nestml::update(nest::Time const & origin, const long from, const long to)
{
  ...
  for ( long lag = from ; lag < to ; ++lag ) // loops through each timestep. core loop for the update function 
  {

    // expression determines the NEST timestep, converting it into NEST object nest::time:step for ms 
    auto get_t = [origin, lag](){ return nest::Time( nest::Time::step( origin.get_steps() + lag + 1) ).get_ms(); };

      // conditional debugger, if loggingtype=DEBUG, we can print the following std::cout 
#ifdef DEBUG
    ...
    for (long i = 0; i < NUM_SPIKE_RECEPTORS; ++i) // buffered values with the receptor ports, storing it in a grid 
    {
    ...
    **/
    ...
    // use of __tmp_ on both displays it's a temp value 



    // CSE implementation - common expression values 
    // These expressions are identical in both the refractory and
    // non-refractory branches.
    // They are calculated BEFORE GSL modifies S_.ode_state.
    // Therefore they can safely be computed once and restored after
    // numerical integration.
    const double V_th_alpha_1__tmp =
        S_.ode_state[State_::V_th_alpha_1]
        * V_.__P__V_th_alpha_1__V_th_alpha_1;
    
    const double V_th_alpha_2__tmp =
        S_.ode_state[State_::V_th_alpha_2]
        * V_.__P__V_th_alpha_2__V_th_alpha_2;
    
    
    const double V_m__tmp =
          V_.__P__V_m__E_L_offset
    
        + S_.ode_state[State_::I_kernel_exc__X__exc_spikes]
            * V_.__P__V_m__I_kernel_exc__X__exc_spikes
    
        + S_.ode_state[State_::I_kernel_inh__X__inh_spikes]
            * V_.__P__V_m__I_kernel_inh__X__inh_spikes
    
        + S_.ode_state[State_::V_m]
            * V_.__P__V_m__V_m
    
        + (P_.I_e + B_.continuous_inputs_grid_sum_[I_STIM])
            * V_.__P__V_m__I_input;
  ...
  if (S_.ode_state[State_::refr_t] > 0) // if neuron is currently in refractory 
  {  
    ...
    const double V_th_alpha_2__tmp = S_.ode_state[State_::V_th_alpha_2] * V_.__P__V_th_alpha_2__V_th_alpha_2; // threshold adaptation variables for alpha1 and alph2 

    // numeric solver: integrating state variables: V_th_alpha_1, V_th_alpha_2, V_m, V_th_v_aux, V_th_v, refr_t to solve for v_m
    // here we solve for V_M but ref_t is not included. 
    ...
    // repeat of V_.__P__V_m__V_m being called. seperate between cse and algebraic simplification 

      
    double __t = 0;
    B_.__sys.function = amat_neuron_nestml_dynamics_V_m_V_th_alpha_1_V_th_alpha_2_V_th_v_V_th_v_aux_refr_t;

      // since we are using an AMAT neuronal model, which their ODE's are linear we should be using a analytical solver
      // only. charl hasn't updated the ode-toolbox for this, but just to keep in mind. 
    ...
    while ( __t < B_.__step ) // keep the GSL library to keep integrating until __t >= B_.__step
    {
  ...
  else // IF THE NEURON IS NOT REFRACTORY 
  {  
    ...
    // still update V_m but we have the ref_t here now. 
    const double V_m__tmp = (-P_.E_L) * V_.__P__V_m__V_m + P_.E_L + S_.ode_state[State_::I_kernel_exc__X__exc_spikes] * V_.__P__V_m__I_kernel_exc__X__exc_spikes + S_.ode_state[State_::I_kernel_inh__X__inh_spikes] * V_.__P__V_m__I_kernel_inh__X__inh_spikes + S_.ode_state[State_::V_m] * V_.__P__V_m__V_m - P_.I_e * V_.__P__V_m__V_m * P_.tau_m / P_.C_m + P_.I_e * P_.tau_m / P_.C_m - B_.continuous_inputs_grid_sum_[I_STIM] * V_.__P__V_m__V_m * P_.tau_m / P_.C_m + B_.continuous_inputs_grid_sum_[I_STIM] * P_.tau_m / P_.C_m;
    ...
    **/
    ...
    // after integration convolutional state is committed. those tmp values from beginning are committed
    S_.ode_state[State_::V_th_alpha_1] = V_th_alpha_1__tmp;
    S_.ode_state[State_::V_th_alpha_2] = V_th_alpha_2__tmp;
    S_.ode_state[State_::V_m] = V_m__tmp;
    ...
    **/
    ...
    //  These represent the effect of incoming spikes. So the convolution variable does approximately:
    ...
    **/
    ...
    /// IF neuron isn't refractory ANDmembrane voltage >= thresholdTHENemit spike
    if (S_.ode_state[State_::refr_t] <= 0 && S_.ode_state[State_::V_m] >= get_V_th())
    {
      ...
      nest::SpikeEvent se;
      nest::kernel().event_delivery_manager.send(*this, se, lag); // the point where the neuron sends the spike event to the connected neurons 
    ...
    **/
    ...
    {
      get_continuous_inputs_grid_sum_()[i] = get_continuous_inputs_()[i].get_value(lag); // contininous inputs, this reads contininous input values. 
    }
    // voltage logging
