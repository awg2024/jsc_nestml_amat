...
#include "amat_neuron.h"

// ---------------------------------------------------------------------------
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<amat_neuron> amat_neuron::recordablesMap_;
namespace nest
{
  // Override the create() method with one call to RecordablesMap::insert_() for each quantity to be recorded.
template <> void RecordablesMap<amat_neuron>::create()
  {
    // add state variables to recordables map
    insert_(amat_neuron_names::_V_th_alpha_1, &amat_neuron::get_V_th_alpha_1);
    insert_(amat_neuron_names::_V_th_alpha_2, &amat_neuron::get_V_th_alpha_2);
    insert_(amat_neuron_names::_V_th_v, &amat_neuron::get_V_th_v);
    insert_(amat_neuron_names::_V_th_v_aux, &amat_neuron::get_V_th_v_aux);
    insert_(amat_neuron_names::_V_m, &amat_neuron::get_V_m);
    insert_(amat_neuron_names::_refr_t, &amat_neuron::get_refr_t);
    insert_(amat_neuron_names::_I_kernel_inh__X__inh_spikes, &amat_neuron::get_I_kernel_inh__X__inh_spikes);
    insert_(amat_neuron_names::_I_kernel_exc__X__exc_spikes, &amat_neuron::get_I_kernel_exc__X__exc_spikes);
    // add recordable inline expressions to recordables map
    insert_(amat_neuron_names::_I_syn_ex, &amat_neuron::get_I_syn_ex);
    insert_(amat_neuron_names::_I_syn_in, &amat_neuron::get_I_syn_in);
    insert_(amat_neuron_names::_I_syn, &amat_neuron::get_I_syn);
    insert_(amat_neuron_names::_V_th, &amat_neuron::get_V_th);
  }
}
std::vector< std::tuple< int, int > > amat_neuron::rport_to_nestml_buffer_idx =
{
  
  { amat_neuron::EXC_SPIKES, amat_neuron::INH_SPIKES },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of amat_neuron's constructor.
// ---------------------------------------------------------------------------

amat_neuron::Parameters_::Parameters_()
{
}

amat_neuron::State_::State_()
{
}
...
amat_neuron::Buffers_::Buffers_(amat_neuron &n):
  logger_(n)
...
amat_neuron::Buffers_::Buffers_(const Buffers_ &, amat_neuron &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
  , spike_input_received_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
...
amat_neuron::amat_neuron():ArchivingNode(), P_(), S_(), B_(*this)
{
...
amat_neuron::amat_neuron(const amat_neuron& __n):
...
// ---------------------------------------------------------------------------
void amat_neuron::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
    "amat_neuron",
...
void amat_neuron::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] amat_neuron::init_state_internal_()" << std::endl;
#endif
...
void amat_neuron::init_buffers_()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] amat_neuron::init_buffers_()" << std::endl;
#endif
  // spike input buffers
  ...
  // B_.__sys.function = amat_neuron_dynamics; // will be set just prior to the call to gsl_odeiv_evolve_apply()
...
void amat_neuron::recompute_internal_variables(bool exclude_timestep)
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
void amat_neuron::pre_run_hook()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] amat_neuron::pre_run_hook()" << std::endl;
#endif
...
extern "C" inline int amat_neuron_dynamics_V_m_V_th_alpha_1_V_th_alpha_2_V_th_v_V_th_v_aux_refr_t(double __time, const double ode_state[], double f[], void* pnode)
{
  typedef amat_neuron::State_ State_;
   // get access to node so we can almost work as in a member function
  assert( pnode );
  const amat_neuron& node = *( reinterpret_cast< amat_neuron* >( pnode ) );
  constexpr int I_STIM = amat_neuron::I_STIM;
...
extern "C" inline int amat_neuron_dynamics_V_m_V_th_alpha_1_V_th_alpha_2_V_th_v_V_th_v_aux(double __time, const double ode_state[], double f[], void* pnode)
{
  typedef amat_neuron::State_ State_;
   // get access to node so we can almost work as in a member function
  assert( pnode );
...
void amat_neuron::update(nest::Time const & origin, const long from, const long to)
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
#ifdef DEBUG
    std::cout << "[neuron " << this << "] amat_neuron::update: handling post spike at t = " << get_t() << std::endl;
#endif
    /**
  ...
  {  
    ...
    double __t = 0;
    B_.__sys.function = amat_neuron_dynamics_V_m_V_th_alpha_1_V_th_alpha_2_V_th_v_V_th_v_aux_refr_t;
  ...
  {  
    ...
    double __t = 0;
    B_.__sys.function = amat_neuron_dynamics_V_m_V_th_alpha_1_V_th_alpha_2_V_th_v_V_th_v_aux;
    // numerical integration with adaptive step size control:
    // ------------------------------------------------------
    // gsl_odeiv_evolve_apply performs only a single numerical
    // integration step, starting from t and bounded by step;
...
void amat_neuron::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
...
void amat_neuron::handle(nest::SpikeEvent &e)
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] amat_neuron::handle(SpikeEvent)" << std::endl;
#endif
  ...
  else
  {
    ...
    if ( nestml_buffer_idx != amat_neuron::PORT_NOT_AVAILABLE )
    {
      weight = -weight;
    }
    else
...
void amat_neuron::handle(nest::CurrentEvent& e)
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] amat_neuron::handle(CurrentEvent)" << std::endl;
#endif
  assert(e.get_delay_steps() > 0);
