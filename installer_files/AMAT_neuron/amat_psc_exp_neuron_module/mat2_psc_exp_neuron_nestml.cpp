...
template <> void RecordablesMap<mat2_psc_exp_neuron_nestml>::create()
  {
   ...
   insert_(mat2_psc_exp_neuron_nestml_names::_V_th_v, &mat2_psc_exp_neuron_nestml::get_V_th_v);
   insert_(mat2_psc_exp_neuron_nestml_names::_V_th_v_aux, &mat2_psc_exp_neuron_nestml::get_V_th_v_aux);
...
mat2_psc_exp_neuron_nestml::mat2_psc_exp_neuron_nestml():ArchivingNode(), P_(), S_(), B_(*this)
{
...
void mat2_psc_exp_neuron_nestml::init_buffers_()
{
  ...
  // B_.__sys.function = mat2_psc_exp_neuron_nestml_dynamics; // will be set just prior to the call to gsl_odeiv_evolve_apply()
...
extern "C" inline int mat2_psc_exp_neuron_nestml_dynamics_refr_t(double __time, const double ode_state[], double f[], void* pnode)
{
  typedef mat2_psc_exp_neuron_nestml::State_ State_;
  // get access to node so we can almost work as in a member function
  assert( pnode );
  const mat2_psc_exp_neuron_nestml& node = *( reinterpret_cast< mat2_psc_exp_neuron_nestml* >( pnode ) );
  constexpr int I_STIM = mat2_psc_exp_neuron_nestml::I_STIM;
  ...
  return GSL_SUCCESS;
}extern "C" inline int mat2_psc_exp_neuron_nestml_dynamics_V_m_V_th_v_V_th_v_aux(double __time, const double ode_state[], double f[], void* pnode)
{
  typedef mat2_psc_exp_neuron_nestml::State_ State_;
  // get access to node so we can almost work as in a member function
  assert( pnode );
...
void mat2_psc_exp_neuron_nestml::update(nest::Time const & origin, const long from, const long to)
{
  ...
  {  
    ...
    double __t = 0;
    B_.__sys.function = mat2_psc_exp_neuron_nestml_dynamics_refr_t;
  ...
  {  
    ...
    double __t = 0;
    B_.__sys.function = mat2_psc_exp_neuron_nestml_dynamics_V_m_V_th_v_V_th_v_aux;
    // numerical integration with adaptive step size control:
    // ------------------------------------------------------
    // gsl_odeiv_evolve_apply performs only a single numerical
    // integration step, starting from t and bounded by step;
